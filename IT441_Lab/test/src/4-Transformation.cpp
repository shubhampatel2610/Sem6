...
void update(std::vector<helpers::Pixel>& pixels)
{
    ...
    //Polygon drawing
    static std::vector<helpers::Point> PolyPoints = { {6.f,18.f},{10.f,10.f},{16.f,16.f},{12.f,24.f} };
    //{10.f,10.f},{10.f,20.f},{20.f,20.f},{20.f,10.f}
    //{6.f,18.f},{10.f,10.f},{16.f,16.f},{12.f,24.f}
    int i = 0;
    ...
    //translation
    if (input::Keyboard::KeyDown(input::Keys::MKEY_T))
    {
        std::cout << "Enter the values of tx and ty : ";
        int tx = 0, ty = 0;
        std::cin >> tx >> ty;
    ...
    //scaling
    else if (input::Keyboard::KeyDown(input::Keys::MKEY_S))
    {
        std::cout << "Enter the values of sx and sy : ";
        float sx = 1, sy = 1;
        std::cin >> sx >> sy;
    ...
    //rotation
    if (input::Keyboard::KeyDown(input::Keys::MKEY_R))
    {
        std::cout << "Enter the value of angle : ";
        float angle;
        std::cin >> angle;
...
int main(int argc, char* args[]) {
    ...
    std::cout << "T for Translation__S for scaling__R for rotation" << std::endl;

    while (!quit)
    {
        memset(&helpers::pixels[0], 0, helpers::pixels.size() * sizeof(helpers::Pixel));
        SDL_WaitEvent(&event);
